...
export async function POST(request: Request) {
  ...
  if (action === 'upsert_shareholder') {
    ...
    const { data: all } = await supabase.from('shareholders').select('id, legal_name, id_or_reg_number, shares_held').eq('entity_id', entityId)
    if (all && all.length > 0) {
      const total = all.reduce((sum, s) => sum + (s.shares_held ?? 0), 0)
      if (total > 0) {
        const recomputed = all.map((s) => ({ ...s, pct: Math.round(((s.shares_held ?? 0) / total) * 10000) / 100 }))
        await Promise.all(
          recomputed.map((s) =>
            supabase.from('shareholders').update({ share_percentage: s.pct }).eq('id', s.id)
          )
        )

        // A shareholder added as a beneficial owner has their own,
        // separately-stored share_percentage — a snapshot taken the
        // moment "add as beneficial owner" was tapped, per BO-conclusions-
        // must-be-user-confirmed (see StepBeneficialOwners). Saving a
        // DIFFERENT shareholder afterward shifts everyone's percentages
        // (shares_held ratios are relative), but that snapshot never
        // moved with it — so a beneficial owner recorded when they held
        // 100% still showed 100% after a second shareholder joined and
        // brought them down to 50% (reported live, 2026-09-12). Refresh
        // the number on any beneficial_owners row still traceable to a
        // shareholder by name or ID/reg number; leave a custom
        // nature-of-control description alone but refresh it too when
        // it still reads exactly like the auto-generated "Shareholding
        // of X%" string, so it doesn't go stale next to the number it's
        // describing.
        const { data: linkedBOs } = await supabase
          .from('beneficial_owners')
          .select('id, full_name, id_number, share_percentage, nature_of_control')
          .eq('entity_id', entityId)
        if (linkedBOs && linkedBOs.length > 0) {
          await Promise.all(
            linkedBOs.map((bo) => {
              const match = recomputed.find((s) =>
                s.legal_name.trim().toLowerCase() === bo.full_name.trim().toLowerCase() ||
                (!!bo.id_number && !!s.id_or_reg_number && s.id_or_reg_number === bo.id_number)
              )
              if (!match || match.pct === bo.share_percentage) return Promise.resolve()
              const wasAutoGenerated = bo.nature_of_control === `Shareholding of ${bo.share_percentage ?? '—'}%`
              return supabase.from('beneficial_owners').update({
                share_percentage: match.pct,
                nature_of_control: wasAutoGenerated ? `Shareholding of ${match.pct}%` : bo.nature_of_control,
              }).eq('id', bo.id)
            })
          )
        }
